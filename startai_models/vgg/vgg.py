...
def vgg_conv_block(inp_channels, out_channels, with_bn=False):
    """Vgg Conv Layer"""
    conv_block = [
        startai.Conv2D(inp_channels, out_channels, [3, 3], 1, 1),
        startai.ReLU(),
    ]
    if with_bn:
        bn_layer = startai.BatchNorm2D(out_channels, training=False)
...
def vgg_block(inp_channel, out_channel, repeat, with_bn=False):
    ...
    layers.append(startai.MaxPool2D(2, 2, 0))
    return layers
...
class VGG(BaseModel):
    ...
    def _build(self, *args, **kwargs):
        ...
        self.classifier = startai.Sequential(
            startai.Linear(7 * 7 * 512, 4096),
            startai.ReLU(),
            startai.Dropout(prob=0.5, training=self.training),
    ...
    def _forward(self, inputs, data_format):
        ...
        if data_format == "NCHW":
            inputs = startai.permute_dims(inputs, (0, 2, 3, 1))
        inputs = self.features(inputs)
        inputs = startai.permute_dims(inputs, (0, 3, 2, 1)).reshape((inputs.shape[0], -1))
