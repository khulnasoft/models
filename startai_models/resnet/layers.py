...
def conv1x1(in_planes: int, out_planes: int, stride: int = 1) -> startai.Conv2D:
    """1x1 convolution"""
    return startai.Conv2D(in_planes, out_planes, [1, 1], stride, 0, with_bias=False)


class BasicBlock(startai.Module):
    """
    ...
    Args::
        ...
        downsample (Optional[startai.Module]): Downsample module for the block.
    ...
    def __init__(
        ...
        planes: int,
        stride: int = 1,
        downsample: Optional[startai.Module] = None,
        base_width: int = 64,
        dilation: int = 1,
    ) -> None:
        self.norm_layer = startai.BatchNorm2D
        if dilation > 1:
...
class Bottleneck(startai.Module):
    """
