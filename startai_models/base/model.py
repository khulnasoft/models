...
class BaseModel(startai.Module):
    def __init__(self, *args, **kwargs):
        super(BaseModel, self).__init__(*args, **kwargs)

    def __setattr__(self, key, value):
        prev_call = inspect.getframeinfo(inspect.currentframe().f_back)[0]
        from_test = "startai_models_tests" in prev_call
        from_startai_module = "startai/stateful/module" in prev_call
        if (
            key == "v"
            and "v" in self.__dict__.keys()
            and self.__dict__["v"] is not None
            and not (from_test or from_startai_module)
        ):
            startai.Container.cont_assert_identical_structure([self.v, value])
    ...
    def load_from_huggingface(
        ...
        if load_model_object:
            ...
            weights = startai.Container.cont_from_disk_as_hdf5(weights_path)
            os.remove(weights_path)
