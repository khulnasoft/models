...
class InceptionConvBlock(startai.Module):
...
class InceptionBlock(startai.Module):
    def __init__(
        self,
        in_channels,
        num1x1,
    ...
    def _forward(self, x):
        ...
        # pool_proj
        pool_proj = startai.max_pool2d(x, [3, 3], 1, 1, ceil_mode=True, data_format="NCHW")
        pool_proj = self.pool_proj_conv(pool_proj)

        ret = startai.concat([conv_1x1, conv_3x3_red, conv_5x5_red, pool_proj], axis=1)
        return ret


class InceptionAuxiliaryBlock(startai.Module):
    ...
    def _build(self, *args, **kwargs):
        ...
        self.fc1 = startai.Linear(2048, 1024, with_bias=True)
        self.dropout = startai.Dropout(self.aux_dropout)
        self.fc2 = startai.Linear(1024, self.num_classes, with_bias=True)
        self.softmax = startai.Softmax()

    def _forward(self, x):
        out = startai.adaptive_avg_pool2d(x, [4, 4])
        out = self.conv(out)
        out = startai.flatten(out, start_dim=1)
        out = self.fc1(out)
        out = startai.relu(out)
