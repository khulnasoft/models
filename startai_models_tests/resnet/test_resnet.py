...
def test_resnet_img_classification(device, fw):
    ...
    # Cardinality test
    assert output.shape == tuple([startai.to_scalar(batch_shape), num_classes])

    # Value test
    if load_weights:
        output = output[0]
        true_indices = startai.array([282, 281, 285])
        calc_indices = startai.argsort(output, descending=True)[:3]

        assert np.array_equal(true_indices, calc_indices)

        true_logits = LOGITS[model_var]
        calc_logits = np.take(
            helpers.np_softmax(startai.to_numpy(output)), startai.to_numpy(calc_indices)
        )
